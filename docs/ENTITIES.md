...
| `is_corporate` | Корпоративный workspace: скрыт из личных списков/итогов; аналитика и операции только в карточке счёта (создание/редактирование не уходит на вкладку «Операции») |
...
| `items` | Список `GoalItem` (позиции цели) |
| `icon`, `color` | Оформление |
| `planned_monthly_contribution` | Плановый вклад / мес. |
| `closed_early` | Закрыта досрочно |

**`GoalItem`:** `name`, `target_amount`, `current_amount`, `status` (open/closed), опциональные метаданные.
...
Платежи отражаются транзакциями с `debt_id`. Список операций для UI может фильтроваться по `has_debt` (не поле сущности — фильтр репозитория).
...
- **`Budget`** — лимит на категорию за месяц/год (валюта базы); опциональный `account_id` для корпоративного счёта (`None` = личный).
