...
| UX helpers | `test_count_up.py`, `test_frequent_account.py`, `test_form_keyboard.py`, `test_ui_motion.py` (web skip fade, overlay `ignore_interactions`), `test_tx_filters_panel.py` (`visible_list_rows`), `test_responsive.py` (viewport cache + content_inset clamp + 320/360/375 nav), `test_app_shell.py` (Column nav ≤420, resize event cache), `test_page_header.py` (no wrap+Expanded on xs) |
